...
def gen_layout():
    ...
    layout = create_card(title,'ov_dt',color,icon)
           
    return layout


@app_PHAI_THU.callback(
    Output("ov_dt", "children"),
...
def UPDATE_OV_DT(start_date,end_date,ddl_npt_kh,ddl_npt_hd,ddl_npt_ct,grh_NPT_KH,active_cell_hd,active_cell_ct,detail_HD,detail_CT):
    ...
    datatable = {'detail_HD':detail_HD,
                 'detail_CT':detail_CT }
    label, value = GParams.Get_Value(ctx,datatable)
    df = DB_PThu.GET_PHAI_THU(('Tien_OV_NPTHU_test',start_date,end_date,ddl_npt_kh,ddl_npt_hd,ddl_npt_ct,label,value,None,None,None))
